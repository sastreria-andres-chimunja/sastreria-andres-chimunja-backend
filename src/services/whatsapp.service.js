...
/**
 * Envía un documento (PDF) a un número de WhatsApp usando Meta Cloud API.
 * Requiere WHATSAPP_TOKEN y WHATSAPP_PHONE_ID en el .env.
 *
 * @param {string} telefono     Número colombiano (10 dígitos, sin +57)
 * @param {Buffer} pdfBuffer    El PDF como Buffer
 * @param {string} filename     Nombre de archivo que verá el cliente en WhatsApp
 * @param {string} caption      Texto que acompaña el documento en WhatsApp
 */
export async function enviarDocumentoWhatsApp(telefono, pdfBuffer, filename, caption) {
  const token   = process.env.WHATSAPP_TOKEN;
  const phoneId = process.env.WHATSAPP_PHONE_ID;
  ...
  // ── Paso 1: Subir el PDF a Meta Media API ──────────────────────────────
  const form = new FormData();
  form.append('messaging_product', 'whatsapp');
